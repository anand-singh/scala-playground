...
object CatsIntegration {
  ...
  case class RegisteredUser(email: String, username: String, passwordHash: String, age: Int)
    ...
    implicit val validatedTransformer: TransformerF[Validated[NonEmptyChain[String], +*], RegistrationForm, RegisteredUser] = {
      Transformer.defineF[Validated[NonEmptyChain[String], +*], RegistrationForm, RegisteredUser]
        ...
        .withFieldComputedF(_.age, form => form.age.toIntOption match {
          ...
          case Some(_) => Validated.invalid(NonEmptyChain(s"${form.username}'s age: must have at least 18 years"))
          case None => Validated.invalid(NonEmptyChain(s"${form.username}'s age: invalid number"))
        })
        .buildTransformer
    }

  val regForm: RegistrationForm = RegistrationForm("john@example.com", "John", "password", "20")
